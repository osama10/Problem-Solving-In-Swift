class Solution {
  func rearrangeString(_ s: String, _ k: Int) -> String {
    ...
    let freqTable = s.reduce(into: [Character: Int]()) { $0[$1, default: 0] += 1 }
    var maxHeap = Heap<(char: Character, freq: Int)> { $0.freq > $1.freq }
    
    for (char, freq) in freqTable {
        maxHeap.insert((char, freq))
    }
    
    var queue = [(char: Character, freq: Int)]()
    var result = ""
    
    while !maxHeap.isEmpty {
        let top = maxHeap.remove()!
        result.append(top.char)
        queue.append((top.char, top.freq - 1))
        
        if queue.count == k {
            let prevElement = queue.removeFirst()
            if prevElement.freq > 0 {
                maxHeap.insert(prevElement)
            }
        }
    }
