...
func isOneEditDistance(_ s: String, _ t: String) -> Bool {
    ...
    if s.count == t.count {
        return checkReplaceCase(Array(s), Array(t))
    } else {
        if s.count < t.count {
...
func deleteAndInsertCase(_ bigStr: [Character], _ smallStr: [Character]) -> Bool {
    ...
    while smallIndex <= smallStr.count - 1 && bigIndex <= bigStr.count - 1 && count <= 1{
...
func checkReplaceCase(_ str1: [Character], _ str2: [Character]) -> Bool {
